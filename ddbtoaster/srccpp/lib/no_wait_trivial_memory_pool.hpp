#ifndef DBTOASTER_NO_WAIT_TRIVIAL_MEMORY_POOL_HPP
#define DBTOASTER_NO_WAIT_TRIVIAL_MEMORY_POOL_HPP
...
static_assert(N > 0, "Chunk size must be greater than 0");
 public:
  ...
  NoWaitTrivialMemoryPool(std::size_t thread_count = omp_get_max_threads()) : thread_count_{thread_count} {
    thread_entries_ = new ThreadEntry[thread_count]();
  }

  ~NoWaitTrivialMemoryPool() {
    releaseAll();
...
void NoWaitTrivialMemoryPool<T, N>::allocateChunk(std::size_t owner) {
  ThreadEntry& entry = thread_entries_[owner];
...
#endif // DBTOASTER_NO_WAIT_TRIVIAL_MEMORY_POOL_HPP
